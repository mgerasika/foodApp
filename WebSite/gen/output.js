...
FoodApp.Common.ngHistoryEntry.prototype.toString = function (){
...
FoodApp.Common.ngHistoryGroupEntry.prototype.toString = function (){
    return this.DateStr;
};
...
FoodApp.Client.ngReviewController.prototype.requestGetUsers = function (){
    FoodApp.Common.JsService.Inst.UsersApi.GetUsers($CreateAnonymousDelegate(this, function (users){
        ...
        for (var $i3 = 0,$l3 = users.length,ngUser = users[$i3]; $i3 < $l3; $i3++, ngUser = users[$i3]){
            var email = ngUser.Email;
            if (-1 != email.indexOf("darwins") || -1 != email.indexOf("stiystil")){
                tmp.push(ngUser);
            }
        }
        if (tmp.length > 0){
            this.requestGetOrders(tmp.reverse());
        }
    }));
};
...
FoodApp.Client.ngHistoryModel.prototype.toString = function (){
    return this.Email;
};
...
FoodApp.Client.ngHistoryModel.prototype.GroupByDate = function (){
    var res = new System.Collections.Generic.List$1.ctor("FoodApp.Common.ngHistoryGroupEntry");
    this.Entries = System.Linq.Enumerable.ToList$1("FoodApp.Common.ngHistoryEntry", System.Linq.Enumerable.OrderBy$2$$IEnumerable$1$$Func$2("FoodApp.Common.ngHistoryEntry", System.DateTime.ctor, this.Entries, $CreateAnonymousDelegate(this, function (o){
        return o.Date;
    })));
...
FoodApp.Client.ngHistoryController.prototype.get_ngUsers = function (){
    return this._scope["ngUsers"];
};
FoodApp.Client.ngHistoryController.prototype.set_ngUsers = function (value){
    this._scope["ngUsers"] = value;
};
FoodApp.Client.ngHistoryController.prototype.get_ngUserId = function (){
    return this._scope["ngUserId"];
};
FoodApp.Client.ngHistoryController.prototype.set_ngUserId = function (value){
    this._scope["ngUserId"] = value;
};
FoodApp.Client.ngHistoryController.prototype.checkMoneyClick = function (userId){
    this.set_ngUserId(userId);
    this.refreshHistory();
};
FoodApp.Client.ngHistoryController.prototype.init = function (scope, http, loc, filter){
    FoodApp.Client.ngControllerBase.prototype.init.call(this, scope, http, loc, filter);
    this.set_ngHistoryItems( []);
    this.set_ngUserId(FoodApp.Client.ngAppController.inst.get_ngUserId());
    this.set_ngUsers( []);
    FoodApp.Common.eventManager.inst.subscribe(FoodApp.Common.eventManager.settingsLoaded, $CreateAnonymousDelegate(this, function (n){
        this.refreshHistory();
    }));
...
FoodApp.Client.ngHistoryController.prototype.refreshHistory = function (){
    FoodApp.Common.jsUtils.inst.showLoading();
    FoodApp.Common.ajaxHlp.inst.SendGet("json", "api/history/" + this.get_ngUserId() + "/", $CreateAnonymousDelegate(this, function (o, s, arg3){
        this.set_ngHistoryItems(o);
        this.createLineChart();
        this.createBarChart();
        this.createTerribleLineChart();
        this._scope.$apply();
        FoodApp.Common.jsUtils.inst.hideLoading();
    }), $CreateDelegate(this, this.onRequestFailed));
};
FoodApp.Client.ngHistoryController.prototype.createLineChart = function (){
    var ctx = document.getElementById("lineChart");
    var allDays =  [];
    for (var $i17 = 0,$t17 = this.get_ngHistoryItems(),$l17 = $t17.length,ngHistoryGroupEntry = $t17[$i17]; $i17 < $l17; $i17++, ngHistoryGroupEntry = $t17[$i17]){
        var date = new Date(ngHistoryGroupEntry.DateStr);
        var price = this.CalcTotalPrice(ngHistoryGroupEntry);
        var item = {
            x: date,
            y: price
        };
    ...
    eval("createChart(ctx,settings)");
};
FoodApp.Client.ngHistoryController.prototype.createTerribleLineChart = function (){
    var ctx = document.getElementById("line2Chart");
    var simpleDays =  [];
    var fridayDays =  [];
    for (var $i18 = 0,$t18 = this.get_ngHistoryItems(),$l18 = $t18.length,ngHistoryGroupEntry = $t18[$i18]; $i18 < $l18; $i18++, ngHistoryGroupEntry = $t18[$i18]){
        var date = new Date(ngHistoryGroupEntry.DateStr);
        var price = this.CalcTotalPrice(ngHistoryGroupEntry);
        var item = {
            x: date,
...
FoodApp.Client.ngHistoryController.prototype.createBarChart = function (){
    var ctx = document.getElementById("barChart");
    var labels =  [];
    var colors =  [];
    var allDays =  [];
    for (var $i19 = 0,$t19 = this.get_ngHistoryItems(),$l19 = $t19.length,ngHistoryGroupEntry = $t19[$i19]; $i19 < $l19; $i19++, ngHistoryGroupEntry = $t19[$i19]){
        var date = new Date(ngHistoryGroupEntry.DateStr);
        var price = this.CalcTotalPrice(ngHistoryGroupEntry);
...
FoodApp.Client.ngHistoryController.prototype.CalcTotalPrice = function (ngHistoryGroupEntry){
    var price = 0;
    var ngHistoryEntries = ngHistoryGroupEntry.Entries;
    for (var $i20 = 0,$l20 = ngHistoryEntries.length,entry = ngHistoryEntries[$i20]; $i20 < $l20; $i20++, entry = ngHistoryEntries[$i20]){
        var str = (entry.Count * entry.FoodPrice);
        var tmp = parseFloat(str);
        price += tmp;
    }
    return price;
};
FoodApp.Client.ngHistoryController.prototype.requestGetUsers = function (complete){
    FoodApp.Common.JsService.Inst.UsersApi.GetUsers($CreateAnonymousDelegate(this, function (res){
        var tmp =  [];
        for (var $i21 = 0,$l21 = res.length,user = res[$i21]; $i21 < $l21; $i21++, user = res[$i21]){
            var email = user.Email;
            if (-1 != email.indexOf("darwins") || -1 != email.indexOf("stiystil")){
                tmp.push(user);
            }
...
FoodApp.Client.ngOrderFilter.prototype.filter = function (obj, arg){
    ...
    if (tmp != null && tmp.length > 0){
        for (var $i22 = 0,$l22 = tmp.length,order = tmp[$i22]; $i22 < $l22; $i22++, order = tmp[$i22]){
...
FoodApp.Client.ngUsersSettingsModel.prototype.GetFoodRateById = function (foodId){
    var res = null;
    var $it22 = this.FoodRates.GetEnumerator();
    while ($it22.MoveNext()){
        var r = $it22.get_Current();
...
FoodApp.Client.ngUsersSettingsModel.prototype.Fix = function (){
    ...
    var $it23 = this.FoodRates.GetEnumerator();
    while ($it23.MoveNext()){
        var foodRate = $it23.get_Current();
    ...
    var $it24 = ratesThatNeedRemove.GetEnumerator();
    while ($it24.MoveNext()){
        var rateToRemove = $it24.get_Current();
...
FoodApp.Client.ngFoodController.prototype.findFoodById = function (id){
    var res = null;
    for (var $i26 = 0,$t26 = this.get_ngFoods(),$l26 = $t26.length,dayItems = $t26[$i26]; $i26 < $l26; $i26++, dayItems = $t26[$i26]){
        for (var $i27 = 0,$l27 = dayItems.length,item = dayItems[$i27]; $i27 < $l27; $i27++, item = dayItems[$i27]){
            if (item.FoodId == id){
                res = item;
                break;
            }
...
FoodApp.Client.ngOrderController.prototype.getTotalPrice = function (day){
    ...
    if (ngOrderModels != null){
        for (var $i28 = 0,$l28 = ngOrderModels.length,item = ngOrderModels[$i28]; $i28 < $l28; $i28++, item = ngOrderModels[$i28]){
...
FoodApp.Client.ngOrderController.prototype.refreshOrders = function (){
    FoodApp.Common.JsService.Inst.OrderApi.GetAllOrders(FoodApp.Common.JsService.Inst.OrderApi.getUserId(), $CreateAnonymousDelegate(this, function (data){
        ...
        for (var $i29 = 0,$l29 = tmp.length,obj = tmp[$i29]; $i29 < $l29; $i29++, obj = tmp[$i29]){
...
FoodApp.Client.ngOrderController.prototype.getOrderByFoodId = function (day, foodId){
    var res = null;
    var orders = FoodApp.Client.ngOrderController.inst.getOrders(day);
    for (var $i30 = 0,$l30 = orders.length,order = orders[$i30]; $i30 < $l30; $i30++, order = orders[$i30]){
...
FoodApp.ngOrderFilter.prototype.filter = function (obj, arg){
    ...
    var tmp = allOrders[day];
    if (tmp != null && tmp.length > 0){
        for (var $i31 = 0,$l31 = tmp.length,order = tmp[$i31]; $i31 < $l31; $i31++, order = tmp[$i31]){
            var foodItem = FoodApp.Client.ngFoodController.inst.findFoodById(order.FoodId);
            if (null != foodItem && !foodItem.isContainer){
                res.push(order);
            }
